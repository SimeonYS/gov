...
class GovPipeline:
    ...
    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `gov`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM gov WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO gov (date, title, link, content)"
